...
def detect_arrows(frame: np.ndarray) -> list[dict]:
    ...
    for cnt in contours:
        ...
        # ── Arrow tip ──────────────────────────────────────────────────────
        # The defect midpoint sits at the base of the arrowhead.
        # The centroid sits back toward the tail.
        # So the vector  centroid → defect_midpoint  points toward the head.
        # We pick the hull point that projects furthest along that direction —
        # this correctly ignores back corners that are geometrically far away
        ...
        centroid_arr = np.array([cx, cy], dtype=float)
        head_dir = mid_defect - centroid_arr
        head_dir /= np.linalg.norm(head_dir) + 1e-6   # unit vector toward head
        projections = (hull_pts - centroid_arr) @ head_dir
        tip = tuple(hull_pts[int(np.argmax(projections))].tolist())
